...
export default function Header({ openContact }) {
  ...
  useEffect(() => {
    setDropOpen(false); setMob(false); setMobSvcOpen(false);
  }, [location.pathname]);

  const goTo = (path) => {
    navigate(path);
    window.scrollTo({ top:0, behavior:'instant' });
    setMob(false); setDropOpen(false); setMobSvcOpen(false);
  };
  ...
  return (
    <>
      <style>{`
        ...
        .nav-svc-btn::after { content:''; position:absolute; bottom:-3px; left:0; width:0; height:1.5px; background:var(--B); transition:width .3s cubic-bezier(.4,0,.2,1); }
        ...
        .nav-dropdown {
          ...
          transform:translateX(-50%) translateY(-10px); width:580px;
        ...
        .nav-dropdown.show { opacity:1; pointer-events:all; transform:translateX(-50%) translateY(0); }
        ...
        .mob-svc-toggle { font-family:var(--FM); font-weight:800; font-size:clamp(32px,6vw,52px); color:var(--Y); letter-spacing:.04em; background:none; border:none; cursor:pointer; text-transform:uppercase; transition:all .3s; display:flex; align-items:center; gap:14px; }
        ...
        .mob-svc-list { display:flex; flex-direction:column; gap:4px; max-height:0; overflow:hidden; transition:max-height .4s cubic-bezier(.4,0,.2,1); width:100%; }
        .mob-svc-list.open { max-height:600px; }
        .mob-svc-item { display:flex; align-items:center; gap:10px; padding:10px 20px; cursor:pointer; background:rgba(240,200,69,.05); border:1px solid rgba(240,200,69,.08); font-family:var(--FM); font-size:14px; font-weight:600; color:var(--Y); text-decoration:none; transition:all .2s; width:90%; }
      ...
      <div className={`mob-menu ${mob ? 'open' : ''}`}>
        ...
        <div className={`mob-svc-list ${mobSvcOpen ? 'open' : ''}`}>
          {SERVICES.map(s => (
            <Link key={s.k} to={s.url} className="mob-svc-item"
