...
const CLIENTS = [
  { name: 'NovaBrand India',    logo: 'NB', industry: 'E-Commerce',        img: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=600&auto=format&fit=crop&q=80', result: '10x ROI',       color: '#f0c845' },
  { name: 'LuxeThreads',        logo: 'LT', industry: 'Fashion & Lifestyle',img: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&auto=format&fit=crop&q=80', result: '340% Sales',     color: '#f0c845' },
  { name: 'TechNest Solutions', logo: 'TN', industry: 'B2B SaaS',           img: 'https://images.unsplash.com/photo-1504868584819-f8e8b4b6d7e3?w=600&auto=format&fit=crop&q=80', result: '225x Traffic',  color: '#f0c845' },
  { name: 'GrowFast',           logo: 'GF', industry: 'SaaS / Tech',        img: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=600&auto=format&fit=crop&q=80', result: '-60% CPL',      color: '#f0c845' },
  { name: 'EduSpark Academy',   logo: 'ES', industry: 'EdTech',             img: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=600&auto=format&fit=crop&q=80', result: '225x Blog',     color: '#f0c845' },
  { name: 'WellnessHive',       logo: 'WH', industry: 'Health & Wellness',  img: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=600&auto=format&fit=crop&q=80', result: '98% Retention', color: '#f0c845' },
  { name: 'BrandCraft Co.',     logo: 'BC', industry: 'D2C Brand',          img: 'https://images.unsplash.com/photo-1559136555-9303baea8ebd?w=600&auto=format&fit=crop&q=80', result: '4x Recall',      color: '#f0c845' },
  { name: 'DigitalX Media',     logo: 'DX', industry: 'Media & Publishing', img: 'https://images.unsplash.com/photo-1533750349088-cd871a92f312?w=600&auto=format&fit=crop&q=80', result: '3x Traffic',    color: '#f0c845' },
  { name: 'NexaRise',           logo: 'NR', industry: 'Fintech',            img: 'https://images.unsplash.com/photo-1611532736597-de2d4265fba3?w=600&auto=format&fit=crop&q=80', result: '8x ROAS',       color: '#f0c845' },
];
...
function Counter({ n, s, label, delay = 0 }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: 'center' }}>
      <div style={{ fontFamily: 'var(--FM)', fontWeight: 900, fontSize: 'clamp(44px,5vw,72px)', color: 'var(--Y)', lineHeight: 1 }}>{num}{s}</div>
      <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(240,200,69,.4)', marginTop: '6px' }}>{label}</div>
    </div>
  );
}
...
function TestimonialSlider() {
  ...
  const goTo = (idx) => { if (animating) return; setAnimating(true); setTimeout(() => { setActive(idx); setAnimating(false); }, 300); };
  useEffect(() => {
    timerRef.current = setInterval(() => { setAnimating(true); setTimeout(() => { setActive(a => (a + 1) % CLIENT_TESTIMONIALS.length); setAnimating(false); }, 300); }, 5000);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ background: 'var(--B4)', border: '1px solid rgba(240,200,69,.1)', padding: '40px', position: 'relative', zIndex: 1, clipPath: 'polygon(0 0,calc(100% - 20px) 0,100% 20px,100% 100%,0 100%)', opacity: animating ? 0 : 1, transform: animating ? 'translateY(12px)' : 'none', transition: 'opacity .3s ease, transform .3s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '24px' }}>
          <img src={t.img} alt={t.name} loading="lazy" decoding="async" style={{ width: '52px', height: '52px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--Y)', filter: 'saturate(.6)' }} />
          <div>
        ...
        <p style={{ fontFamily: 'var(--FI)', fontStyle: 'italic', fontSize: 'clamp(14px,1.6vw,18px)', lineHeight: 1.75, color: 'rgba(240,200,69,.7)', marginBottom: '0' }}>{t.quote}</p>
      </div>
      <div style={{ display: 'flex', gap: '8px', marginTop: '18px', justifyContent: 'center' }}>
        {CLIENT_TESTIMONIALS.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} style={{ width: i === active ? '36px' : '20px', height: '3px', background: i === active ? 'var(--Y)' : 'rgba(240,200,69,.2)', border: 'none', cursor: 'pointer', transition: 'all .4s cubic-bezier(.22,1,.36,1)' }} />
        ))}
      </div>
    </div>
  );
...
function ClientCard({ client, delay = 0 }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ position: 'relative', overflow: 'hidden', cursor: 'default', aspectRatio: '4/3', opacity: 0, transform: 'translateY(28px)', animation: `clientCardIn .6s ${delay}s cubic-bezier(.22,1,.36,1) forwards` }}>
      <img src={client.img} alt={client.name} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: hovered ? 'saturate(.7) contrast(1.05)' : 'saturate(.15) contrast(1.2)', transform: hovered ? 'scale(1.07)' : 'scale(1)', transition: 'filter .6s ease, transform .6s ease' }} />
      <div style={{ position: 'absolute', inset: 0, background: hovered ? 'linear-gradient(to top,rgba(14,16,75,.96) 0%,rgba(14,16,75,.3) 60%,transparent 100%)' : 'linear-gradient(to top,rgba(14,16,75,.88) 0%,rgba(14,16,75,.55) 100%)', transition: 'background .5s ease' }} />
      <div style={{ position: 'absolute', top: '14px', left: '14px', width: '42px', height: '42px', borderRadius: '50%', background: 'var(--Y)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--FM)', fontWeight: 900, fontSize: '13px', color: 'var(--B)', border: '2px solid rgba(255,255,255,.15)', transform: hovered ? 'scale(1.1)' : 'scale(1)', transition: 'transform .4s cubic-bezier(.34,1.56,.64,1)', zIndex: 2 }}>{client.logo}</div>
      <div style={{ position: 'absolute', top: '14px', right: '14px', fontSize: '8px', fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(240,200,69,.7)', background: 'rgba(14,16,75,.7)', border: '1px solid rgba(240,200,69,.2)', padding: '3px 8px', zIndex: 2 }}>{client.industry}</div>
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 18px', zIndex: 2 }}>
        <div style={{ fontFamily: 'var(--FM)', fontWeight: 800, fontSize: 'clamp(14px,1.4vw,18px)', color: 'var(--Y)', marginBottom: '6px', transform: hovered ? 'translateY(0)' : 'translateY(4px)', transition: 'transform .4s ease' }}>{client.name}</div>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'var(--Y)', color: 'var(--B)', fontFamily: 'var(--FM)', fontWeight: 800, fontSize: '12px', padding: '4px 12px', transform: hovered ? 'translateY(0) scale(1)' : 'translateY(8px) scale(.9)', opacity: hovered ? 1 : 0, transition: 'all .35s cubic-bezier(.34,1.56,.64,1)' }}>✦ {client.result}</div>
      </div>
    </div>
  );
}
      ...
      <div style={{ display: 'flex', width: 'max-content', animation: 'logoScroll 22s linear infinite' }}>
        {[...names,...names,...names].map((n,i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '22px', padding: '0 28px', whiteSpace: 'nowrap' }}>
            <span style={{ fontFamily: 'var(--FM)', fontWeight: 800, fontSize: '13px', letterSpacing: '.12em', color: 'rgba(240,200,69,.22)', textTransform: 'uppercase', transition: 'color .3s' }} onMouseEnter={e => e.currentTarget.style.color='rgba(240,200,69,.7)'} onMouseLeave={e => e.currentTarget.style.color='rgba(240,200,69,.22)'}>{n}</span>
    ...
    <>
      <Helmet>
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          "@type": "WebPage",
          "name": "Ereynard Clients — 200+ Brands Scaled",
          "description": "200+ Indian brands scaled by Ereynard across E-Commerce, SaaS, EdTech, Fashion, Fintech and D2C.",
          "url": "https://www.ereynard.com/clients",
          "publisher": {
            "@type": "Organization",
            "name": "Ereynard",
      ...
      <style>{`
        @keyframes clientCardIn { from{opacity:0;transform:translateY(28px);}to{opacity:1;transform:translateY(0);} }
        @keyframes logoScroll { from{transform:translateX(0);}to{transform:translateX(-33.33%);} }
        @keyframes heroTextIn { from{opacity:0;transform:translateY(32px);}to{opacity:1;transform:none;} }
        @keyframes heroBadgeIn { from{opacity:0;transform:scale(.8) rotate(-6deg);}to{opacity:1;transform:scale(1) rotate(0);} }
        .filter-btn{font-family:var(--FM);font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:7px 16px;border:1px solid rgba(240,200,69,.18);background:none;color:rgba(240,200,69,.45);cursor:pointer;transition:all .25s;white-space:nowrap;}
        .filter-btn:hover{border-color:rgba(240,200,69,.4);color:var(--Y);}
        .filter-btn.active{background:var(--Y);color:var(--B);border-color:var(--Y);}
        .trust-item{display:flex;align-items:flex-start;gap:14px;padding:22px;border:1px solid rgba(240,200,69,.07);background:var(--B4);transition:all .35s;}
        .trust-item:hover{border-color:rgba(240,200,69,.22);background:rgba(240,200,69,.05);transform:translateY(-4px);}
        @media(max-width:1100px){.clients-grid{grid-template-columns:repeat(2,1fr)!important;}.stats-strip{grid-template-columns:repeat(2,1fr)!important;}.trust-grid{grid-template-columns:1fr 1fr!important;}.bottom-grid{grid-template-columns:1fr!important;}}
        @media(max-width:640px){.clients-grid{grid-template-columns:1fr!important;}.filter-row{gap:6px!important;}.filter-btn{font-size:9px;padding:5px 10px;}}
      `}</style>
      ...
      <div style={{ background:'var(--Y)' }}>
        <div className="stats-strip" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)' }}>
          {STATS.map((s,i) => (
            <div key={i} style={{ padding:'32px 20px', borderRight: i < STATS.length-1 ? '1px solid rgba(14,16,75,.12)':'none', position:'relative', overflow:'hidden' }} onMouseEnter={e=>e.currentTarget.style.background='rgba(14,16,75,.05)'} onMouseLeave={e=>e.currentTarget.style.background='none'}>
      ...
      <div style={{ background:'var(--B)' }}>
        <section className="sec on-navy">
          ...
          <div className="filter-row" style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginTop:'28px', marginBottom:'32px' }}>
            {INDUSTRIES.map(ind => (
              <button key={ind} className={`filter-btn ${activeFilter===ind?'active':''}`} onClick={()=>setActiveFilter(ind)}>{ind}</button>
            ))}
          </div>
          <div className="clients-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'3px' }}>
            {filtered.map((c,i) => <ClientCard key={c.name} client={c} delay={i*0.07} />)}
          </div>
          {filtered.length===0 && <div style={{ textAlign:'center', padding:'60px', color:'rgba(240,200,69,.3)', fontFamily:'var(--FM)' }}>No clients in this category yet 🦊</div>}
        </section>
      </div>
            ...
            <div>
              ...
              <div className="trust-grid reveal" style={{ display:'grid', gridTemplateColumns:'1fr', gap:'2px', marginTop:'24px' }}>
                {[
                  ...
                  { i:'🦊', t:'The Fox Method',          d:'4-step framework: Discover → Strategise → Execute → Optimise.' },
      ...
      <div style={{ background:'var(--Y)', padding:'72px 56px' }}>
        <div style={{ maxWidth:'800px', margin:'0 auto', textAlign:'center' }}>
          <div style={{ fontSize:'9px', fontWeight:700, letterSpacing:'.28em', textTransform:'uppercase', color:'rgba(14,16,75,.42)', marginBottom:'14px' }}>Join 200+ Brands</div>
          <h2 style={{ fontFamily:'var(--FM)', fontWeight:900, fontSize:'clamp(32px,4.5vw,64px)', color:'var(--B)', lineHeight:1.05, marginBottom:'14px' }}>Ready to Become<br />Our Next Success Story?</h2>
          <p style={{ fontSize:'15px', lineHeight:1.75, color:'rgba(14,16,75,.55)', maxWidth:'500px', margin:'0 auto 32px' }}>Free proposal. No commitment. Just a clear plan to grow your brand — delivered in 48 hours.</p>
