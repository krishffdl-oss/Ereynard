...
import { SVC_CARD_ICONS, FEAT_ICONS } from './abc.jsx';
      ...
      <div className="svc-modal">
        ...
        <div className="sm-head">
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            {/* SVG icon box instead of emoji */}
            <div style={{
              width: '52px', height: '52px', flexShrink: 0,
              background: 'rgba(14,16,75,.12)',
              border: '1px solid rgba(14,16,75,.2)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: 'var(--B)',
            }}>
              <span style={{ transform: 'scale(1.5)', display: 'flex' }}>
                {SVC_CARD_ICONS[svcKey]}
              </span>
            </div>
            <div>
        ...
        <div className="sm-body">
          ...
          <div className="sm-results">
            {d.res.map((r, i) => (
              <div key={i}>
                <div className="sm-r-n">{r.n}</div>
                <div className="sm-r-l">{r.l}</div>
              </div>
            ))}
          </div>
